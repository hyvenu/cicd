...
class OrderService:
    ...
    def get_orders(self, order_status):
        final_list = []
        order_list = OrderRequest.objects.filter(order_status=order_status) \
            .all().values(
            'id',
            'order_raised_date',
            'customer__id',
    ...
    def process_order(self, order_data, user_id):
        ...
        return order_request.order_number
    ...
    def update_order(self,order_id, order_status):
        ...
        order.save()
        self.track_events(order_id, order_status)
