...
export class CalenderComponent implements OnInit {
  ...
  constructor(private nbtoastService: NbToastrService,
    private adminService:AdminService,) {
      ...
      this.adminService.getAppointmentList().subscribe(
           ...
           this.appointment_list.push(
               {
                 ...
                 end: addHours(new Date(element.booking_date + ' ' + element.end_time), 2),
                 title: content ,
                 color: colors.blue,
                 actions: this.actions,
                 resizable: {
        ...
        (error) => {
          this.nbtoastService.danger("Unable to Load Appointment data");
        }
      )
      this.events = this.appointment_list;
  ...
  handleEvent(action: string, event: CalendarEvent): void {
    // this.modalData = { event, action };
     this.nbtoastService.show(event + '' + action);
