...
export class ManageGrnComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.grnMasterForm = this.formBuilder.group({
      ...
      vehicleNumberFormControl: ['', []],
      timeInFormControl: ['', []],
      timeOutFormControl: ['', []],
      transporterNameFormControl: ['', []],
      statutoryDetailsFormControl: ['', []],
      noteFormControl: ['', []],
    ...
    //  this.selected_product_list = data.product_list;
          this.selected_product_list = [];
           data.product_list.forEach(element => {
            ...
            let obj = this.product_list_all.find(ele => ele.product_name === element.product_name);
  ...
  calculate(event) {
    ...
    if(colId == "amount") {
     ...
     let  unit = event.data.amount / event.data.received_qty;
      ...
      st = (st + Number(event.data.amount));
      console.log("chnaged event called " + this.sub_total);
      this.total_gst = this.total_gst + Number(event.data.gst_amount);

      this.totalQuantityReceived += Number(event.data.received_qty);
    ...
    this.calculate_total();
          ...
          calculate_total(): any {
            ...
            this.selected_product_list.forEach(element => {
              ...
              this.total_gst = this.total_gst +parseFloat(element.gst_amount)
            });
            console.log(this.selected_product_list);
    // this.sub_total = st;
    if (this.vendor_state_code == '29') {
